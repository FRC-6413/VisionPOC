...
public class PixyCmu5 implements AdvancedPIDSource
{
     ...
     */
    public PixyCmu5(I2C i2c_port, Double period, ReentrantLock mutex)
    {
    ...
    /**
     * TODO: Remove this and replace with getTotalDegrees(int nObjects)
    ...
    public synchronized double getTotalDegrees() {
		...
		{
    		degrees = PixyCmu5.degreesXFromCenter(getCurrentframes().get(0));
		} else if(getNumObjectsDetected() >= 2) {
			degrees = (PixyCmu5.degreesXFromCenter(getCurrentframes().get(0)) +
							 PixyCmu5.degreesXFromCenter(getCurrentframes().get(1))) / 2;
    ...
    public synchronized double getTotalDegrees(int nObjects) {
    	...
    	if(getNumObjectsDetected() >= nObjects)
		{
    		...
    		{
    			degrees+=PixyCmu5.degreesXFromCenter(getCurrentframes().get(idx));
    		}
    		degrees /= nObjects;
